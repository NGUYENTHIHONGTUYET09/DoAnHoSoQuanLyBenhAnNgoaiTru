...
public class DangKyGUI extends JFrame {
	...
	//	jbutton_luu.setBackground(Color.GREEN);
		...
		JButton jbutton_huy = new JButton("Thoát");
    ...
    public void performDangKi() throws AddressException, MessagingException {
        ...
        if (jRadioButton_BS.isSelected() || jRadioButton_NVTN.isSelected() || jRadioButton_NVTT.isSelected()
            ...
            if (!maSoTonTai) {
                errorMessage = "Nhân viên chưa có hồ sơ!";
                isValid = false;
            }

            if (!dsdkbus.checkEmailNotExists(email)) {
                errorMessage = "Email đã được đăng ký với 1 tài khoản khác!";
                isValid = false;
            }
            ...
            if (isValid) {
                ...
                // Chỉ khi việc đăng ký thành công thì mới chuyển trang và gửi email
...
//
//			NhanVien nv = null;
//			for (NhanVien n : dsnvbus.getDSFromDB()) {
//				if (n.getMANV().equals(maSo)) {
//					maSoTonTai = true;
//					nv = n;
//					break;
//				}
...
//			}
//
//			if (nv != null && !dsdkbus.checkEmailNotExists(email)) {
//				errorMessage = "Email đã được đăng ký với 1 tài khoản khác!";
//				isValid = false;
//			}
//
...
//
//			if (isValid) {
//				addTaiKhoanNhanVien(selectedRole);
//
//				
//				TrangChuAdmin tca = new TrangChuAdmin();
//				tca.setVisible(true);
